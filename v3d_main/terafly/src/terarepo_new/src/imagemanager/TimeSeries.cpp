...
TimeSeries::TimeSeries(const char *rootDir, std::string frames_format /* = "" */) noexcept(false) : iim::VirtualVolume(rootDir)
{
...
void TimeSeries::initChannels ( ) noexcept(false)
{
    /**/iim::debug(iim::LEV2, 0, __iim__current__function__);
...
std::vector<iim::VirtualVolume*> TimeSeries::getActiveFrames() noexcept(false)
{
    /**/iim::debug(iim::LEV3, 0, __iim__current__function__);
...
iim::VirtualVolume* TimeSeries::getFrameAt(int t) noexcept(false)
{
...
iim::real32 * TimeSeries::loadSubvolume_to_real32(int V0,int V1, int H0, int H1, int D0, int D1)  noexcept(false)
{
...
iim::uint8 * TimeSeries::loadSubvolume_to_UINT8(int V0,int V1, int H0, int H1, int D0, int D1, int *channels /*=0*/, int ret_type /*=iim::DEF_IMG_DEPTH*/) noexcept(false)
{
