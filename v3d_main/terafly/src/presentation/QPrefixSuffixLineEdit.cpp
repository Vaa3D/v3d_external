...
QPrefixSuffixValidator::State QPrefixSuffixValidator::validate ( QString & input, int & pos ) const {
	...
	// So we still have the separator.  What's before it might be bad or good.
        ...
        int indexOfSep = input.indexOf(separator1());
...
void QPrefixSuffixLineEdit::setPrefix(const QString& prefix)
{
    ...
    if(originalText.contains(separator1()))
        originalText.remove(0, originalText.indexOf(separator1()) + 1);

	// ad the new one
    originalText.prepend(separator1());
...
void QPrefixSuffixLineEdit::setSuffix(const QString& suffix)
{
    ...
    if(oldText.contains(separator2()))
        oldText.chop(oldText.length() - oldText.lastIndexOf(separator2()));

    oldText.append(separator2());
