...
static _Tiff_Histogram  Tiff_Histogram_Proto;

static int Tiff_Histogram_Offset = ((char *) &(Tiff_Histogram_Proto.tiff_histogram)) - ((char *) &Tiff_Histogram_Proto);
...
static inline void pack_tiff_histogram(Tiff_Histogram *tiff_histogram)
{ _Tiff_Histogram *object  = (_Tiff_Histogram *) (((char *) tiff_histogram) - Tiff_Histogram_Offset);
}

Tiff_Histogram *Pack_Tiff_Histogram(Tiff_Histogram *tiff_histogram)
{ pack_tiff_histogram(tiff_histogram);
  return (tiff_histogram);
}
...
static _Tiff_Channel  Tiff_Channel_Proto;

static int Tiff_Channel_Offset = ((char *) &(Tiff_Channel_Proto.tiff_channel)) - ((char *) &Tiff_Channel_Proto);
...
{ _Tiff_Channel *object  = (_Tiff_Channel *) (((char *) tiff_channel) - Tiff_Channel_Offset);
  if (tiff_channel->histogram != NULL)
    Pack_Tiff_Histogram(tiff_channel->histogram);
}
...
static _Timage  Timage_Proto;

static int Timage_Offset = ((char *) &(Timage_Proto.timage)) - ((char *) &Timage_Proto);
...
static inline void allocate_timage_channels(Timage *timage, int csize, char *routine)
{ _Timage *object = (_Timage *) (((char *) timage) - Timage_Offset);
  if (object->csize < csize)
    { timage->channels = Guarded_Realloc(timage->channels,csize,routine);
      object->csize = csize;
    }
}
...
static inline void allocate_timage_map(Timage *timage, int msize, char *routine)
{ _Timage *object = (_Timage *) (((char *) timage) - Timage_Offset);
  if (object->msize < msize)
    { timage->map = Guarded_Realloc(timage->map,msize,routine);
      object->msize = msize;
    }
}
...
{ _Timage *object;
  ...
  allocate_timage_channels(timage,csize,routine);
  allocate_timage_map(timage,msize,routine);
  timage->ifd_ref = NULL;
  return (timage);
}
...
{ Timage *copy = new_timage(timage_csize(timage),timage_msize(timage),"Copy_Tiff_Image");
  ...
  *copy = *timage;
  ...
  if (timage->channels != NULL)
    memcpy(copy->channels,timage->channels,timage_csize(timage));
  copy->map = _map;
  if (timage->map != NULL)
    memcpy(copy->map,timage->map,timage_msize(timage));
  if (timage->ifd_ref != NULL)
    Inc_Tiff_IFD(timage->ifd_ref);
  return (copy);
}

static inline void pack_timage(Timage *timage)
{ _Timage *object  = (_Timage *) (((char *) timage) - Timage_Offset);
  if (object->csize > timage_csize(timage))
    { object->csize = timage_csize(timage);
      if (object->csize != 0)
        timage->channels = Guarded_Realloc(timage->channels,
                                           object->csize,"Pack_Timage");
      else
  ...
  if (object->msize > timage_msize(timage))
    { object->msize = timage_msize(timage);
      if (object->msize != 0)
        timage->map = Guarded_Realloc(timage->map,
                                      object->msize,"Pack_Timage");
      else
        { free(timage->map);
          timage->map = NULL;
        }
