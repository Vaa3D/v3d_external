...
bool rotate_along_zaxis(unsigned char* inimg1d, V3DLONG * in_sz, unsigned char* outimg1d , V3DLONG * out_sz, float theta)
{
	if(inimg1d == 0 || in_sz[0] <=0 || in_sz[1] <= 0 || in_sz[2] <= 0 || in_sz[3] <= 0 || out_sz != 0) return false;
	...
	for(int i = 0; i < width * height * depth * channels; i++) outimg1d[i] = 0;
