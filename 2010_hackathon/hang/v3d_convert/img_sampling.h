...
// down sampling without blur
template<class T> bool down_sampling_without_blur(int factor, T* &inimg1d, V3DLONG * &in_sz, T* &outimg1d, V3DLONG * &out_sz)
{
...
template<class T> bool down_sampling(int factor, T* &inimg1d, V3DLONG * &in_sz, T* &outimg1d, V3DLONG * &out_sz)
{
	...
	for(V3DLONG k = 0; k < out_sz[2]; k++)
	{
		V3DLONG ks = k * factor, ke = (k+1) * factor;
		for(V3DLONG j = 0; j < out_sz[1]; j++)
		{
			V3DLONG js = j * factor, je = (j+1) * factor;
			for(V3DLONG i = 0; i < out_sz[0]; i++)
			{
				V3DLONG is = i * factor, ie = (i+1) * factor;
				double sum_int = 0.0;
				for(V3DLONG kk = ks; kk < ke; kk++)
				{
					for(V3DLONG jj = js; jj < je; jj++)
					{
						for(V3DLONG ii = is; ii < ie; ii++)
						{
							sum_int += inimg3d[kk][jj][ii];
						}
					}
				}
				outimg3d[k][j][i] =(T)(sum_int/factor/factor/factor + 0.5);
			}
		}
	}
