...
template <class T> bool computeCircularScatteringScore(Vector1DSimple<double> & meanVec, Vector1DSimple<double> & cntVec, Vector1DSimple<double> & scatterVec, Vol3DSimple<T> *img, V3DLONG x0, V3DLONG y0, V3DLONG z0, int r)
{
    ...
    double * t = meanVec.getData1dHandle();
    double * cnt = cntVec.getData1dHandle();
	double * sscore = scatterVec.getData1dHandle();
	V3DLONG i,j,k;
	...
	vector <Coord3D_JBA> * vecArray = new vector <Coord3D_JBA> [r+1];
	if (!vecArray)
	{
		fprintf(stderr, "Fail to allocate memory in computeCircularScatteringScore(). \n");
		return false;
	}
		...
		for (j=jb;j<=je;j++)
		{
			...
			for (i=ib;i<=ie;i++)
			{
				...
				if (data3d[k][j][i]>0.1)
				{
					t[cur_index] += data3d[k][j][i];
					cnt[cur_index] += 1;
					vecArray[cur_index].push_back(Coord3D_JBA(dx, dy, dz));
				}
			}
		}
	}
	...
	for (i=0;i<=r;i++)
	{
		...
		pvec = vecArray+i;
		double scatter_score = 0.0;
		if (pvec->size()<=1)
		{
			scatter_score = 0; //invalid value
		}
		else
		{
		    scatter_score=0.0;
			for (j=0;j<pvec->size();j++)
			{
				...
				{
					c2 = pvec->at(k);
					//dx = pvec->at(j).x - pvec->at(k).x; dx *= dx;
					//dy = pvec->at(j).y - pvec->at(k).y; dy *= dy;
					//dz = pvec->at(j).z - pvec->at(k).z; dz *= dz;
					
					dx = c1.x - c2.x; dx *= dx;
					dy = c1.y - c2.y; dy *= dy;
					dz = c1.z - c2.z; dz *= dz;
					scatter_score += sqrt(dx+dy+dz);
				}
			}
			scatter_score /= (double(pvec->size())*(pvec->size()-1)/2.0);
		}
		
		if (i>0)
			sscore[i] = scatter_score/i;
		
		//printf("%d\t%5.3f\t%5.3f\t%5.3f\n", i, t[i], cnt[i], sscore[i]);
	}
	//printf("--------------------------------\n");
	
	if (vecArray) {delete []vecArray; vecArray=0;}
