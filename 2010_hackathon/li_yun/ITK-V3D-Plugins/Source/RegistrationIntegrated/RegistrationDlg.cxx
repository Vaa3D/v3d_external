...
RegistrationDlg::RegistrationDlg()
{
	...
	//connect(MetricMethod,SIGNAL(currentIndexChanged(int)),this,SLOT(updateMtcOptim(int)));
...
void RegistrationDlg::updateMtcOptim(int i)
{
	std::cout<<"i: "<<i<<std::endl;
	if(i == 4)
	{
...
void RegistrationDlg::SetFinalParameters(QList<double> list)
{
	...
	if(transform == "AffineTransform")
	{
		...
		for(i=0;i<3;i++)
		{
			...
			matRow1 = QString::number(list.at(i*14+5),'g',6)+QString::number(list.at(i*14+6),'g',6)+QString::number(list.at(i*14+7),'g',6);
			matRow1 = QString::number(list.at(i*14+8),'g',6)+QString::number(list.at(i*14+9),'g',6)+QString::number(list.at(i*14+10),'g',6);
